use crate::{USERS, check_cycles_before_action, DataPoint, Model, User};
...
#[ic_cdk::update]
pub(crate) fn calculate_statistical_parity_difference(model_id: u128) -> f32 {
...
#[ic_cdk::update]
pub(crate) fn calculate_disparate_impact(model_id: u128) -> f32 {
    check_cycles_before_action();
    USERS.with(|users: &RefCell<HashMap<Principal, User>>| {
        let mut users: std::cell::RefMut<'_, HashMap<Principal, User>> = users.borrow_mut();
        let user: &mut User = users
...
#[ic_cdk::update]
pub(crate) fn calculate_average_odds_difference(model_id: u128) -> f32 {
    check_cycles_before_action();
    USERS.with(|users: &RefCell<HashMap<Principal, User>>| {
        let mut users: std::cell::RefMut<'_, HashMap<Principal, User>> = users.borrow_mut();
        let user: &mut User = users
...
#[ic_cdk::update]
pub(crate) fn calculate_equal_opportunity_difference(model_id: u128) -> f32 {
    check_cycles_before_action();
    USERS.with(|users| {
...
#[ic_cdk::update]
pub(crate) fn calculate_all_metrics(model_id: u128) -> (f32, f32, f32, f32, f32, f32, f32) {
...
pub(crate) fn calculate_group_counts(data_points: &Vec<DataPoint>) -> (i128, i128, i128, i128) {
...
pub(crate) fn calculate_overall_confusion_matrix(data_points: &Vec<DataPoint>) -> (i128, i128, i128, i128) {
